...
def run_evaluation(agent_name, data_dir, num_trials=10, gallery_size=10, query_size=1, api_key=None, report_path="report.md", model="gpt-4o", base_url=None, use_memory=True):
    ...
    if agent_name == "mock":
        ...
        if not api_key:
            ...
            return
        # 从 config 中读取 backend，默认为 openai 以获得更好的稳定性
        backend = config.get("backend", "openai")
        agent = EvolutionaryReIDAgent(api_key=api_key, model=model, base_url=base_url, backend=backend, use_memory=use_memory)
    else:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(config_path):
        # Try parent directory if not found in current (useful if running from src/)
    ...
    data_dir = args.data_dir or config.get("data_dir", os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data/1/Market-1501-v15.09.15"))
