...
SKETCH_GUIDANCE = """
    CRITICAL INSTRUCTION: THIS IS A CROSS-MODAL SKETCH-TO-PHOTO TASK.
    The Query image is a hand-drawn black-and-white SKETCH. 
    The Gallery contains color PHOTOS.
    
    YOU MUST OBEY THE FOLLOWING RULES:
    1. ZERO COLOR BIAS: Do NOT use color as a matching criterion. The color in the photos cannot be inferred from the sketch.
    2. FOCUS ON TOPOLOGY: Look strictly at structural features (e.g., long coat vs short jacket, skirt vs pants, collar shape).
    3. CHECK ACCESSORIES: Look for the presence of backpacks, handbags, glasses, hats, or specific hair styles (e.g., ponytails).
    4. IGNORE BACKGROUND: Ignore the background environment and lighting completely.
"""
...
class PairWiseLocalAgent(BaseReIDAgent):
    def __init__(self, api_key="local-test", model="/data/llm/AI-ModelScope/R-4B", base_url="http://localhost:8000/v1"):
        ...
        print(f"🚀 Pair-wise API 模式已激活，日志将保存至: {self.log_file}")
    ...
    def predict(self, query_paths, gallery_paths, guidance="无"):
        ...
        if guidance and guidance != "无" and guidance.strip() != "":
            guidance_block = f"### 🚨 TASK SPECIFIC GUIDANCE 🚨\n{guidance}\n*SYSTEM WARNING: Strictly adhere to the above guidance!*"
        else:
            guidance_block = ""
        ...
        for idx, g_path in enumerate(gallery_paths):
            ...
            prompt_text = textwrap.dedent(f"""\
                ...
                Your task is to determine if the person shown in the [Query Images] and the Gallery Candidate image contain the EXACT SAME person.
                ...
                ### Step 1: Synthesize Query Profile
                - Combined Query Profile: [Synthesize the clothing structure and accessories into ONE complete profile.]
                ...
                - Synthesized Query: [Exact sleeve length, visible patterns, clothing type (IGNORE color if sketch task)]
                - Gallery Candidate: [Exact sleeve length, visible patterns, clothing type]
                - Contradiction?: [Yes/No]
            ...
            try:
                ...
                with open(self.log_file, "a", encoding="utf-8") as f:
                    f.write(f"### 🖼️ Candidate {idx} ({os.path.basename(g_path)}) -> **{status_text}**\n")
                    f.write(f"> {res_content.replace(chr(10), '<br>')}\n\n")

            except Exception as e:
                results_list.append({"idx": idx, "is_match": False, "confidence": 0}) 
                analyses.append(f"Error: {e}")
                print(f"   -> ❌ G-{idx} 发生错误: {e}")
...
class ListWiseLocalAgent(BaseReIDAgent):
    ...
    def predict(self, query_paths, gallery_paths, guidance="无"):
        ...
        prompt_text = textwrap.dedent(f"""\
            ...
            1. Describe the key structural features, clothing styles, and accessories of the Query person.
            2. Visually scan the Gallery Grid. Eliminate candidates with obvious structural mismatches (or color mismatches ONLY IF color is allowed).
        ...
        content = [{"type": "text", "text": prompt_text}] + query_contents + [
            ...
            {"type": "text", "text": "Take a deep breath and carefully look at the images. Remember to output [MATCH_INDEX]: <number> at the very end."}
        ]
...
class EvolutionaryReIDAgent(BaseReIDAgent):
    ...
    def visual_reflect(self, query_paths, eval_paths, error_reason, task_type):
        """合并了FP和FN的反思方法，节省代码"""
        query_b64 = self.base_agent._encode_image(query_paths[0])
        eval_b64 = [self.base_agent._encode_image(p) for p in eval_paths]

        prompt = textwrap.dedent(f"""\
            你是一个严厉的刑侦视觉督导。下属在行人重识别时犯错了！
            错误类型：{task_type} (FP=把路人当目标，FN=漏认了真正的目标)
            下属的错误推理录音：{error_reason}
            
            请批判他的推理过程，并总结出一条强硬、简短的【视觉排查铁律】。必须以 `[GUIDANCE]:` 开头输出。
        """)
        
        content = [{"type": "text", "text": prompt}, {"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{query_b64}"}}]
        for b64 in eval_b64:
            content.append({"type": "image_url", "image_url": {"url": f"data:image/jpeg;base64,{b64}"}})
            
        try:
            response = self.base_agent.client.chat.completions.create(
                model=self.base_agent.model, messages=[{"role": "user", "content": content}], max_tokens=512, temperature=0.6 
            )
            if hasattr(response, 'usage') and response.usage:
                self.base_agent.total_tokens_used += response.usage.total_tokens
            res_content = response.choices[0].message.content.strip()
            match = re.search(r'\[GUIDANCE\]:\s*(.*)', res_content, re.IGNORECASE)
            return match.group(1) if match else "交叉核对结构细节，拒绝幻觉！"
        except Exception as e:
            return "仔细检查细节！"

    def predict(self, query_paths, gallery_paths, ground_truth_idx=None, guidance="无"):
        # 【核心修正】：将外部传进来的 Sketch Guidance 和自己进化的 Memory Guidance 结合！
        memory_guidance = self._get_current_guidance()
        combined_guidance = (guidance if guidance else "") + "\n" + memory_guidance
        
        print(f"\n[Predictor] 正在执行单目标识别 (融合了外部设定与自我记忆)...")
        ...
        if ground_truth_idx is not None:
            ...
            if not is_correct:
                if pred_idx != -1:
                    print("\n[Reflector] 检测到错认(FP)，触发反思...")
                    new_guidance = self.visual_reflect(query_paths, [gallery_paths[pred_idx], gallery_paths[ground_truth_idx]], analyses_list[pred_idx], "FP(认错人)")
                else:
                    print("\n[Reflector] 检测到漏认(FN)，触发反思...")
                    new_guidance = self.visual_reflect(query_paths, [gallery_paths[ground_truth_idx]], analyses_list[ground_truth_idx], "FN(没认出)")
                
                if new_guidance:
                    print(f"✨ [Memory] 习得新规矩: {new_guidance}")
                    self.active_guidance_list.append(new_guidance)
            
            self.persistent_memory.add_trial(prompt=combined_guidance, is_correct=is_correct, analysis=f"Pred: {pred_idx}, GT: {ground_truth_idx}")
