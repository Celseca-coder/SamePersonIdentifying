...
class LocalReIDAgent(BaseReIDAgent):
    def __init__(self, api_key="local-test", model="/data/llm/AI-ModelScope/R-4B", base_url="http://localhost:8000/v1"):
        ...
        # 不在这里删除日志，避免覆盖，追加记录即可
        print(f"🚀 vLLM API (Predictor) 已激活。")
    ...
    def predict(self, query_path, gallery_paths, guidance="无"):
        ...
        for idx, g_path in enumerate(gallery_paths):
            ...
            # 在您的 Prompt 顶部强行注入历史教训！
            prompt_text = textwrap.dedent(f"""\
                # SYSTEM INSTRUCTION: CRITICAL OUTPUT CONSTRAINT
                
                【🚨来自过去的深刻教训 (GUIDANCE)🚨】: 
                {guidance}
                请你务必在本次分析中严格遵守上述教训，不要重蹈覆辙！
                
                You are a highly analytical AI vision expert specializing in Pedestrian Re-Identification.
                Your task is to determine if the Query image and the Gallery Candidate image contain the EXACT SAME person.

                CRITICAL RULES FOR REID TASK:
                    1. ANTI-HALLUCINATION CONSTRAINT: NEVER invent visual features. 
                    2. HARD NEGATIVE REJECTION: Actively search for irreconcilable structural differences.
                    3. FINE-GRAINED GRANULARITY: Specify exact color shades and materials.
                    4. VIEWPOINT INVARIANCE: Ignore background environments.
            ...
            try:
                response = self.client.chat.completions.create(
                    ...
                    messages=[{"role": "user", "content": content}],
                    max_tokens=1024, 
                    temperature=0.1 # Predictor需要极度冷静
                )
...
class EvolutionaryReIDAgent(BaseReIDAgent):
    ...
    def predict(self, query_path, gallery_paths, ground_truth_idx=None):
        ...
        # 此时 pred_indices 是一个列表，例如 [1, 3] 或者 []
        pred_indices, analyses_list = self.base_agent.predict(query_path, gallery_paths, guidance=current_guidance)
        
        if ground_truth_idx is not None:
            # 统一把标准答案也包装成列表，方便处理
            if not isinstance(ground_truth_idx, list):
                gt_indices = [ground_truth_idx]
            else:
                gt_indices = ground_truth_idx

            # 【核心逻辑】：使用集合计算 FP(错认) 和 FN(漏认)
            pred_set = set(pred_indices)
            gt_set = set(gt_indices)
            
            false_positives = pred_set - gt_set  # 选了但不在答案里的（认错人了）
            false_negatives = gt_set - pred_set  # 在答案里但没选的（漏认了目标）
            
            # 只有既没有错认，也没有漏认，才算 100% 完美正确
            ...
            if not is_correct:
                ...
                # 错误 1：处理“认错人（False Positive）”
                if false_positives:
                    wrong_idx = random.choice(list(false_positives)) 
                    target_gt = random.choice(list(gt_indices)) 
                    
                    wrong_path = gallery_paths[wrong_idx]
                    correct_path = gallery_paths[target_gt]
                    wrong_reason = analyses_list[wrong_idx]
                    correct_reason = analyses_list[target_gt]
                    
                    print("\n[Reflector] 检测到错认(FP)，触发排雷反思...")
                    guidance_fp = self.visual_reflect_wrong_match(
                        query_path, wrong_path, correct_path, wrong_reason, correct_reason
                    )
                    if guidance_fp:
                        new_guidances.append(guidance_fp)
                    
                # 错误 2：独立处理“漏认了（False Negative）”
                # 注意：这里改成了 if，不再是 elif！如果同时犯错，两个都会执行！
                if false_negatives:
                    missed_idx = random.choice(list(false_negatives)) 
                    correct_path = gallery_paths[missed_idx]
                    missed_reason = analyses_list[missed_idx]
                    
                    print("\n[Reflector] 检测到漏认(FN)，触发召回反思...")
                    guidance_fn = self.visual_reflect_miss_hit(
                        query_path, correct_path, missed_reason
                    )
                    if guidance_fn:
                        new_guidances.append(guidance_fn)
                
                # 将新习得的所有规矩加入记忆库
                for g in new_guidances:
                    print(f"✨ [Memory] 习得新规矩: {g}")
                    self.active_guidance_list.append(g)
            
            # 将列表转为字符串记录到日志中
            log_analysis = f"Pred: {list(pred_indices)}, GT: {list(gt_indices)}"
